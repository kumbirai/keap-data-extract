...
class CheckpointManager:
    ...
    def save_checkpoint(self, entity_type: str, offset: int) -> None:
        self.checkpoints[entity_type] = offset
    ...
    def get_checkpoint(self, entity_type: str) -> int:
        return self.checkpoints.get(entity_type, 0)
...
def load_contacts(client: KeapClient, db: Session, checkpoint_manager: CheckpointManager, batch_size: int = 50) -> None:
    """Load all contacts and their related data."""
    offset = checkpoint_manager.get_checkpoint('contacts')
    total_contacts = 0
    
    while True:
        logger.info(f"Fetching contacts with offset {offset} and batch size {batch_size}")
        ...
        try:
            if processed_in_batch > 0:
                db.commit()
                # Update checkpoint only for successfully processed contacts
                offset += processed_in_batch
                checkpoint_manager.save_checkpoint('contacts', offset)
...
def load_tags(client: KeapClient, db: Session, checkpoint_manager: CheckpointManager, batch_size: int = 50) -> None:
    """Load all tags."""
    offset = checkpoint_manager.get_checkpoint('tags')
    total_tags = 0
    
    while True:
        logger.info(f"Fetching tags with offset {offset} and batch size {batch_size}")
        tags = client.get_tags(limit=batch_size, offset=offset)
        ...
        try:
            if processed_in_batch > 0:
                db.commit()
                offset += processed_in_batch
                checkpoint_manager.save_checkpoint('tags', offset)
...
def load_custom_fields(client: KeapClient, db: Session, batch_size: int = 50) -> None:
    """Load all custom fields."""
    offset = 0
    total_fields = 0
    while True:
        logger.info(f"Fetching custom fields with offset {offset} and batch size {batch_size}")
        custom_fields = client.get_custom_fields(limit=batch_size, offset=offset)
...
def load_opportunities(client: KeapClient, db: Session, checkpoint_manager: CheckpointManager, batch_size: int = 50) -> None:
    """Load all opportunities."""
    offset = checkpoint_manager.get_checkpoint('opportunities')
    total_opportunities = 0
    
    while True:
        logger.info(f"Fetching opportunities with offset {offset} and batch size {batch_size}")
        opportunities = client.get_opportunities(limit=batch_size, offset=offset)
        ...
        try:
            if processed_in_batch > 0:
                db.commit()
                offset += processed_in_batch
                checkpoint_manager.save_checkpoint('opportunities', offset)
...
def load_products(client: KeapClient, db: Session, checkpoint_manager: CheckpointManager, batch_size: int = 50) -> None:
    """Load all products, including subscription plans."""
    offset = checkpoint_manager.get_checkpoint('products')
    total_products = 0
    
    while True:
        logger.info(f"Fetching products with offset {offset} and batch size {batch_size}")
        products = client.get_products(limit=batch_size, offset=offset)
        ...
        try:
            if processed_in_batch > 0:
                db.commit()
                offset += processed_in_batch
                checkpoint_manager.save_checkpoint('products', offset)
...
def load_orders(client: KeapClient, db: Session, checkpoint_manager: CheckpointManager, batch_size: int = 50) -> None:
    """Load all orders with their items."""
    offset = checkpoint_manager.get_checkpoint('orders')
    total_orders = 0
    
    while True:
        logger.info(f"Fetching orders with offset {offset} and batch size {batch_size}")
        orders = client.get_orders(limit=batch_size, offset=offset)
        ...
        try:
            if processed_in_batch > 0:
                db.commit()
                offset += processed_in_batch
                checkpoint_manager.save_checkpoint('orders', offset)
...
def load_tasks(client: KeapClient, db: Session, checkpoint_manager: CheckpointManager, batch_size: int = 50) -> None:
    """Load all tasks."""
    offset = checkpoint_manager.get_checkpoint('tasks')
    total_tasks = 0
    
    while True:
        logger.info(f"Fetching tasks with offset {offset} and batch size {batch_size}")
        tasks = client.get_tasks(limit=batch_size, offset=offset)
        ...
        try:
            if processed_in_batch > 0:
                db.commit()
                offset += processed_in_batch
                checkpoint_manager.save_checkpoint('tasks', offset)
...
def load_notes(client: KeapClient, db: Session, checkpoint_manager: CheckpointManager, batch_size: int = 50) -> None:
    """Load all notes."""
    offset = checkpoint_manager.get_checkpoint('notes')
    total_notes = 0
    
    while True:
        logger.info(f"Fetching notes with offset {offset} and batch size {batch_size}")
        notes = client.get_notes(limit=batch_size, offset=offset)
        ...
        try:
            if processed_in_batch > 0:
                db.commit()
                offset += processed_in_batch
                checkpoint_manager.save_checkpoint('notes', offset)
...
def load_campaigns(client: KeapClient, db: Session, checkpoint_manager: CheckpointManager, batch_size: int = 50) -> None:
    """Load all campaigns and their sequences."""
    offset = checkpoint_manager.get_checkpoint('campaigns')
    total_campaigns = 0
    
    while True:
        logger.info(f"Fetching campaigns with offset {offset} and batch size {batch_size}")
        campaigns = client.get_campaigns(limit=batch_size, offset=offset)
        ...
        try:
            if processed_in_batch > 0:
                db.commit()
                offset += processed_in_batch
                checkpoint_manager.save_checkpoint('campaigns', offset)
...
def load_subscriptions(client: KeapClient, db: Session, checkpoint_manager: CheckpointManager, batch_size: int = 50) -> None:
    """Load all active subscriptions."""
    offset = checkpoint_manager.get_checkpoint('subscriptions')
    total_subscriptions = 0
    
    while True:
        logger.info(f"Fetching subscriptions with offset {offset} and batch size {batch_size}")
        subscriptions = client.get_subscriptions(limit=batch_size, offset=offset)
        ...
        try:
            if processed_in_batch > 0:
                db.commit()
                offset += processed_in_batch
                checkpoint_manager.save_checkpoint('subscriptions', offset)
...
def main(resume: bool = False):
    ...
    try:
        ...
        load_tags(client, db, checkpoint_manager)
        load_contacts(client, db, checkpoint_manager)
        load_products(client, db, checkpoint_manager)
        load_opportunities(client, db, checkpoint_manager)
        load_orders(client, db, checkpoint_manager)
        load_tasks(client, db, checkpoint_manager)
        load_notes(client, db, checkpoint_manager)
        load_campaigns(client, db, checkpoint_manager)
        load_subscriptions(client, db, checkpoint_manager)
