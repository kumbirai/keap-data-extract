...
class AffiliateRedirect(Base):
    ...
    program_ids = relationship("AffiliateRedirectProgram", back_populates="affiliate_redirect",
...
class CustomField(Base):
    ...
    type = Column(Enum(CustomFieldType), nullable=True)
    options = Column(JSON, nullable=True)  # Array of string options for dropdown/multiselect/radio fields
    created_at = Column(DateTime, default=utc_now)
    modified_at = Column(DateTime, default=utc_now, onupdate=utc_now)

    # Relationships
    values = relationship("ContactCustomFieldValue", back_populates="custom_field", cascade="all, delete-orphan")
    field_metadata = relationship("CustomFieldMetaData", back_populates="custom_field", uselist=False,
                                  cascade="all, delete-orphan")
    opportunity_values = relationship("OpportunityCustomFieldValue", back_populates="custom_field",
                                      cascade="all, delete-orphan")
    order_values = relationship("OrderCustomFieldValue", back_populates="custom_field", cascade="all, delete-orphan")
    subscription_values = relationship("SubscriptionCustomFieldValue", back_populates="custom_field",
                                       cascade="all, delete-orphan")
...
class OpportunityCustomFieldValue(Base):
    ...
    __table_args__ = (
        UniqueConstraint('opportunity_id', 'custom_field_id', name='uix_opportunity_custom_field'),
    )
...
class OrderCustomFieldValue(Base):
    ...
    __table_args__ = (
        UniqueConstraint('order_id', 'custom_field_id', name='uix_order_custom_field'),
    )
...
class SubscriptionCustomFieldValue(Base):
    ...
    __table_args__ = (
        UniqueConstraint('subscription_id', 'custom_field_id', name='uix_subscription_custom_field'),
    )
...
class Opportunity(Base):
    ...
    custom_field_values = relationship("OpportunityCustomFieldValue", back_populates="opportunity",
...
class Subscription(Base):
    ...
    custom_field_values = relationship("SubscriptionCustomFieldValue", back_populates="subscription",
                                       cascade="all, delete-orphan")
...
# Join tables
contact_address = Table('contact_address', Base.metadata,
                        Column('contact_id', Integer, ForeignKey('contacts.id', ondelete='CASCADE'), primary_key=True),
                        Column('address_id', Integer, ForeignKey('addresses.id', ondelete='CASCADE'), primary_key=True),
                        Column('created_at', DateTime, default=utc_now)
                        )

contact_email = Table('contact_email', Base.metadata,
                      Column('contact_id', Integer, ForeignKey('contacts.id', ondelete='CASCADE'), primary_key=True),
                      Column('email_id', Integer, ForeignKey('email_addresses.id', ondelete='CASCADE'),
                             primary_key=True),
                      Column('created_at', DateTime, default=utc_now)
                      )

contact_phone = Table('contact_phone', Base.metadata,
                      Column('contact_id', Integer, ForeignKey('contacts.id', ondelete='CASCADE'), primary_key=True),
                      Column('phone_id', Integer, ForeignKey('phone_numbers.id', ondelete='CASCADE'), primary_key=True),
                      Column('created_at', DateTime, default=utc_now)
                      )

contact_fax = Table('contact_fax', Base.metadata,
                    Column('contact_id', Integer, ForeignKey('contacts.id', ondelete='CASCADE'), primary_key=True),
                    Column('fax_id', Integer, ForeignKey('fax_numbers.id', ondelete='CASCADE'), primary_key=True),
                    Column('created_at', DateTime, default=utc_now)
                    )

contact_opportunity = Table('contact_opportunity', Base.metadata,
...
contact_task = Table('contact_task', Base.metadata,
                     Column('contact_id', Integer, ForeignKey('contacts.id', ondelete='CASCADE'), primary_key=True),
                     Column('task_id', Integer, ForeignKey('tasks.id', ondelete='CASCADE'), primary_key=True),
                     Column('created_at', DateTime, default=utc_now)
                     )

contact_note = Table('contact_note', Base.metadata,
                     Column('contact_id', Integer, ForeignKey('contacts.id', ondelete='CASCADE'), primary_key=True),
                     Column('note_id', Integer, ForeignKey('notes.id', ondelete='CASCADE'), primary_key=True),
                     Column('created_at', DateTime, default=utc_now)
                     )

contact_order = Table('contact_order', Base.metadata,
                      Column('contact_id', Integer, ForeignKey('contacts.id', ondelete='CASCADE'), primary_key=True),
                      Column('order_id', Integer, ForeignKey('orders.id', ondelete='CASCADE'), primary_key=True),
                      Column('created_at', DateTime, default=utc_now)
                      )

contact_subscription = Table('contact_subscription', Base.metadata,
                             Column('contact_id', Integer, ForeignKey('contacts.id', ondelete='CASCADE'),
                                    primary_key=True),
                             Column('subscription_id', Integer, ForeignKey('subscriptions.id', ondelete='CASCADE'),
                                    primary_key=True),
                             Column('created_at', DateTime, default=utc_now)
                             )
...
product_order_item = Table('product_order_item', Base.metadata,
...
product_subscription = Table('product_subscription', Base.metadata,
                             Column('product_id', Integer, ForeignKey('products.id', ondelete='CASCADE'),
                                    primary_key=True),
                             Column('subscription_id', Integer, ForeignKey('subscriptions.id', ondelete='CASCADE'),
                                    primary_key=True),
...
campaign_sequence = Table('campaign_sequence', Base.metadata,
                          Column('campaign_id', Integer, ForeignKey('campaigns.id', ondelete='CASCADE'),
                                 primary_key=True),
                          Column('sequence_id', Integer, ForeignKey('campaign_sequences.id', ondelete='CASCADE'),
                                 primary_key=True),
                          Column('created_at', DateTime, default=utc_now)
                          )
